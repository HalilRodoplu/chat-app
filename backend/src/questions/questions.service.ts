...
export class QuestionsService {
  ...
  async getQuestions(): Promise<Question[]> {
    return this.questionModel.find().exec();  // Veritabanındaki tüm chat'leri getir
  }

  // Yeni chat oluştur
  async createNewChat(): Promise<Question> {
    const lastChat = await this.questionModel
        .findOne()
        .sort({ chatNumber: -1 })
        .exec();

    const newChatNumber = lastChat && lastChat.chatNumber ? lastChat.chatNumber + 1 : 1;

    const newChat = new this.questionModel({
